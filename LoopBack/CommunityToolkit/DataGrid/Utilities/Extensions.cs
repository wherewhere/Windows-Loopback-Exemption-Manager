...
namespace CommunityToolkit.WinUI.Utilities
{
    ...
    {
        private static readonly Dictionary<DependencyObject, Dictionary<DependencyProperty, int>> _suspendedHandlers = [];

        public static bool IsHandlerSuspended(this DependencyObject dependencyObject, DependencyProperty dependencyProperty)
        {
            return _suspendedHandlers.TryGetValue(dependencyObject, out Dictionary<DependencyProperty, int> value) && value.ContainsKey(dependencyProperty);
        }
        ...
        {
            if (_suspendedHandlers.TryGetValue(obj, out Dictionary<DependencyProperty, int> suspensions))
            {
                if (incrementSuspensionCount)
                {
                    if (suspensions.TryGetValue(dependencyProperty, out int value))
                    {
                        suspensions[dependencyProperty] = ++value;
                    }
                    else
                    {
