...
namespace CommunityToolkit.WinUI.Controls.Primitives
{
    ...
    public sealed partial class DataGridRowsPresenter : Panel
    {
        ...
        {
            ManipulationStarting += new ManipulationStartingEventHandler(DataGridRowsPresenter_ManipulationStarting);
            ManipulationStarted += new ManipulationStartedEventHandler(DataGridRowsPresenter_ManipulationStarted);
            ManipulationDelta += new ManipulationDeltaEventHandler(DataGridRowsPresenter_ManipulationDelta);
        }
        ...
        protected override Size ArrangeOverride(Size finalSize)
        {
            ...
            foreach (UIElement element in OwningGrid.DisplayData.GetScrollingElements())
            {
        ...
        protected override Size MeasureOverride(Size availableSize)
        {
            ...
            bool invalidateRows =
                (!OwningGrid.RowsPresenterAvailableSize.HasValue || availableSize.Width != OwningGrid.RowsPresenterAvailableSize.Value.Width) &&
        ...
        private void DataGridRowsPresenter_ManipulationStarting(object sender, ManipulationStartingRoutedEventArgs e)
        {
            if (OwningGrid != null)
            {
                DiagnosticsDebug.Assert(OwningGrid.IsEnabled, "Expected OwningGrid.IsEnabled is true.");

                _preManipulationHorizontalOffset = OwningGrid.HorizontalOffset;
                _preManipulationVerticalOffset = OwningGrid.VerticalOffset;
            }
        }
        ...
        private void DataGridRowsPresenter_ManipulationDelta(object sender, ManipulationDeltaRoutedEventArgs e)
        {
            ...
            {
                e.Handled =
                    OwningGrid.ProcessScrollOffsetDelta(_preManipulationHorizontalOffset - e.Cumulative.Translation.X - OwningGrid.HorizontalOffset, true /*isForHorizontalScroll*/) ||
                    OwningGrid.ProcessScrollOffsetDelta(_preManipulationVerticalOffset - e.Cumulative.Translation.Y - OwningGrid.VerticalOffset, false /*isForHorizontalScroll*/);
            }
        }
        ...
        internal void PrintChildren()
        {
            foreach (UIElement element in Children)
            {
                ...
                if (row != null)
                {
                    DiagnosticsDebug.WriteLine(string.Format(System.Globalization.CultureInfo.InvariantCulture, "Slot: {0} Row: {1} Visibility: {2} ", row.Slot, row.Index, row.Visibility));
                }
                else
                {
                    DataGridRowGroupHeader groupHeader = element as DataGridRowGroupHeader;
                    if (groupHeader != null)
                    {
                        ...
                        DiagnosticsDebug.WriteLine(string.Format(System.Globalization.CultureInfo.InvariantCulture, "Slot: {0} GroupHeader: {1} Visibility: {2}", groupHeader.RowGroupInfo.Slot, groupHeader.RowGroupInfo.CollectionViewGroup.Name, groupHeader.Visibility));
#else
                        DiagnosticsDebug.WriteLine(string.Format(System.Globalization.CultureInfo.InvariantCulture, "Slot: {0} Visibility: {1}", groupHeader.RowGroupInfo.Slot, groupHeader.Visibility));
#endif
                    }
                }
            }
