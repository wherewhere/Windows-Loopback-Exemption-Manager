...
namespace CommunityToolkit.WinUI.Controls
{
    ...
    public partial class DataGridRow : Control
    {
        ...
        public DataGridRow()
        {
            ...
            _computedForeground = Foreground;
            Cells = new DataGridCellCollection(this);
            Cells.CellAdded += new EventHandler<DataGridCellEventArgs>(DataGridCellCollection_CellAdded);
            Cells.CellRemoved += new EventHandler<DataGridCellEventArgs>(DataGridCellCollection_CellRemoved);

            AddHandler(TappedEvent, new TappedEventHandler(DataGridRow_Tapped), true /*handledEventsToo*/);

            PointerCanceled += new PointerEventHandler(DataGridRow_PointerCanceled);
            PointerCaptureLost += new PointerEventHandler(DataGridRow_PointerCaptureLost);
            PointerPressed += new PointerEventHandler(DataGridRow_PointerPressed);
            PointerReleased += new PointerEventHandler(DataGridRow_PointerReleased);
            PointerEntered += new PointerEventHandler(DataGridRow_PointerEntered);
            PointerExited += new PointerEventHandler(DataGridRow_PointerExited);
            PointerMoved += new PointerEventHandler(DataGridRow_PointerMoved);
        ...
        {
            get => GetValue(DetailsTemplateProperty) as DataTemplate;
        ...
        internal bool IsSelected
        {
            get
            {
                ...
                DiagnosticsDebug.Assert(Index != -1, "Expected Index other than -1.");
        ...
        private DataTemplate ActualDetailsTemplate
        {
            ...
            {
                DiagnosticsDebug.Assert(OwningGrid != null, "Expected non-null owning DataGrid.");
                DataTemplate currentDetailsTemplate = DetailsTemplate;

                return currentDetailsTemplate ?? OwningGrid.RowDetailsTemplate;
            }
        }
        ...
        private bool AreDetailsVisible => DetailsVisibility == Visibility.Visible;
        ...
        protected override Size ArrangeOverride(Size finalSize)
        {
            ...
            {
                RectangleGeometry gridlineClipGeometry = new()
                {
                    Rect = new Rect(OwningGrid.HorizontalOffset, 0, Math.Max(0, DesiredSize.Width - OwningGrid.HorizontalOffset), _bottomGridLine.DesiredSize.Height)
                };
        ...
        protected override Size MeasureOverride(Size availableSize)
        {
            ...
            currentAddItemIsDataContext = OwningGrid.DataConnection.EditableCollectionView.CurrentAddItem == DataContext;
#endif
            Size desiredSize;
            try
            {
            ...
            desiredSize.Width = Math.Max(desiredSize.Width, OwningGrid.CellsWidth);
            if (double.IsNaN(Height) && DoubleUtil.IsZero(MinHeight) &&
                (Index == OwningGrid.DataConnection.NewItemPlaceholderIndex ||
                (OwningGrid.DataConnection.IsAddingNew && currentAddItemIsDataContext)))
            {
                ...
                desiredSize.Height = Math.Max(desiredSize.Height, OwningGrid.RowHeightEstimate);
            }
        ...
        {
            RootElement = GetTemplateChild(DATAGRIDROW_elementRoot) as Panel;
            ...
            if (_detailsElement != null && OwningGrid != null)
            {
                ...
                if (ActualDetailsVisibility == Visibility.Visible && ActualDetailsTemplate != null && _appliedDetailsTemplate == null)
                {
        ...
        internal void ApplyDetailsTemplate(bool initializeDetailsPreferredHeight)
        {
            if (_detailsElement != null && AreDetailsVisible)
            {
                ...
                if (ActualDetailsTemplate != _appliedDetailsTemplate)
                {
                    if (ActualDetailsTemplate == null)
                    {
                    ...
                    else
                    {
                        ...
                        _detailsContent = ActualDetailsTemplate.LoadContent() as FrameworkElement;
        ...
        {
            if (RootElement != null && OwningGrid != null && Visibility == Visibility.Visible)
            {
                DiagnosticsDebug.Assert(Index != -1, "Expected Index other than -1.");
        ...
        internal void EnsureForeground()
        {
            ...
            if (OwningGrid != null)
            {
                ...
                var newForeground = Index % 2 == 0 || OwningGrid.AlternatingRowForeground == null
                    ? OwningGrid.RowForeground
                    : OwningGrid.AlternatingRowForeground;

                ComputedForeground = newForeground ?? Foreground;
            }
            else
            {
        ...
        internal void EnsureHeaderVisibility()
        {
            if (_headerElement != null && OwningGrid != null)
            {
                _headerElement.Visibility = OwningGrid.AreRowHeadersVisible ? Visibility.Visible : Visibility.Collapsed;
            }
        }
        ...
        private void DataGridRow_Tapped(object sender, TappedRoutedEventArgs e)
        {
            ...
            {
                if (OwningGrid.UpdatedStateOnTapped)
                {
                    OwningGrid.UpdatedStateOnTapped = false;
                }
                else
                {
        ...
        {
            if (dependencyProperty == ForegroundProperty)
            {
                EnsureForeground();
            }
        }
