    ...
    public partial class DataGrid
    {
        ...
        internal void AutoSizeColumn(DataGridColumn column, double desiredWidth)
        {
            DiagnosticsDebug.Assert(
                column.Width.IsAuto || column.Width.IsSizeToCells || column.Width.IsSizeToHeader || (!UsesStarSizing && column.Width.IsStar),
        ...
        {
            DiagnosticsDebug.Assert(dataGridColumn != null, "Expected non-null dataGridColumn.");
            DataGridColumnEventArgs e = new(dataGridColumn);
        ...
        internal void OnColumnVisibleStateChanged(DataGridColumn updatedColumn)
        {
            ...
            {
                DiagnosticsDebug.Assert(SelectedIndex == DataConnection.IndexOf(SelectedItem), "Expected SelectedIndex equals DataConnection.IndexOf(SelectedItem).");
                if (SelectedIndex != -1)
                {
        ...
        internal void OnColumnVisibleStateChanging(DataGridColumn targetColumn)
        {
            ...
            if (targetColumn.IsVisible && CurrentColumn == targetColumn)
            {
                ...
                DataGridColumn dataGridColumn = ColumnsInternal.GetNextVisibleColumn(targetColumn) ?? ColumnsInternal.GetPreviousVisibleNonFillerColumn(targetColumn);
        ...
        internal void OnInsertedColumn_PostNotification(DataGridCellCoordinates newCurrentCellCoordinates, int newDisplayIndex)
        {
            ...
            {
                DiagnosticsDebug.Assert(CurrentColumnIndex == -1, "Expected CurrentColumnIndex equals -1.");
        ...
        internal void OnInsertedColumn_PreNotification(DataGridColumn insertedColumn)
        {
            ...
            DiagnosticsDebug.Assert(insertedColumn.Index < ColumnsItemsInternal.Count, "insertedColumn.Index smaller than ColumnsItemsInternal.Count.");
        ...
        internal DataGridCellCoordinates OnRemovingColumn(DataGridColumn dataGridColumn)
        {
            ...
            DiagnosticsDebug.Assert(dataGridColumn.Index < ColumnsItemsInternal.Count, "Expected dataGridColumn.Index smaller than ColumnsItemsInternal.Count.");
        ...
        private double AdjustStarColumnWidths(int displayIndex, double adjustment, bool userInitiated)
        {
            ...
            List<DataGridColumn> starColumns = [];
        ...
        private double AdjustStarColumnWidths(int displayIndex, double remainingAdjustment, bool userInitiated, Func<DataGridColumn, double> targetWidth)
        {
            ...
            List<KeyValuePair<DataGridColumn, double>> starColumnPairs = [];
        ...
        private bool ComputeDisplayedColumns()
        {
            ...
            DiagnosticsDebug.Assert(cx <= ColumnsInternal.GetVisibleFrozenEdgedColumnsWidth(), "cx smaller than or equal to ColumnsInternal.GetVisibleFrozenEdgedColumnsWidth().");
...
#if DEBUG
                DiagnosticsDebug.Assert(ColumnsInternal.Debug_VerifyColumnDisplayIndexes(), "Expected ColumnsInternal.Debug_VerifyColumnDisplayIndexes() is true.");
#endif
        ...
        private void CorrectColumnIndexesAfterDeletion(DataGridColumn deletedColumn)
        {
            ...
            for (int columnIndex = deletedColumn.Index; columnIndex < ColumnsItemsInternal.Count; columnIndex++)
            {
                ...
                DiagnosticsDebug.Assert(ColumnsItemsInternal[columnIndex].Index == columnIndex, "Expected ColumnsItemsInternal[columnIndex].Index equals columnIndex.");
            }
        }
        ...
        private void FlushDisplayIndexChanged(bool raiseEvent)
        {
            foreach (DataGridColumn column in ColumnsItemsInternal)
            {
                if (column.DisplayIndexHasChanged)
                {
                    ...
                    if (raiseEvent)
                    {
                        DiagnosticsDebug.Assert(column != ColumnsInternal.RowGroupSpacerColumn, "Expected column other than ColumnsInternal.RowGroupSpacerColumn.");
        ...
        {
            DiagnosticsDebug.Assert(index < ColumnsItemsInternal.Count, "Expected index smaller than ColumnsItemsInternal.Count.");
            DiagnosticsDebug.Assert(ColumnsItemsInternal[index].IsVisible, "Expected ColumnsItemsInternal[index].IsVisible is true.");
        ...
        private void ScrollColumns(int columns)
        {
            ...
            if (columns > 0)
            {
                ...
                DiagnosticsDebug.Assert(DisplayData.FirstDisplayedScrollingCol >= 0, "Expected positive DisplayData.FirstDisplayedScrollingCol.");
