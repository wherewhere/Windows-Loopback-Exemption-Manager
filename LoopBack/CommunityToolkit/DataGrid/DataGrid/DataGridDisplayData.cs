...
namespace CommunityToolkit.WinUI.Controls.DataGridInternals
{
    internal class DataGridDisplayData
    {
        ...
        {
            DiagnosticsDebug.Assert(slot >= FirstScrollingSlot, "Expected slot greater than or equal to FirstScrollingSlot.");
            DiagnosticsDebug.Assert(slot <= LastScrollingSlot, "Expected slot less than or equal to LastScrollingSlot.");
        ...
        internal void LoadScrollingSlot(int slot, UIElement element, bool updateSlotInformation)
        {
            ...
            else
            {
                ...
                DiagnosticsDebug.Assert(slot >= _owner.GetPreviousVisibleSlot(FirstScrollingSlot), "Expected slot greater than or equal to _owner.GetPreviousVisibleSlot(FirstScrollingSlot).");
                DiagnosticsDebug.Assert(slot <= _owner.GetNextVisibleSlot(LastScrollingSlot), "Expected slot smaller than or equal to _owner.GetNextVisibleSlot(LastScrollingSlot).");
        ...
        {
            foreach (UIElement element in GetScrollingElements())
            {
                DataGridRow row = element as DataGridRow;
                if (row != null)
                {
                    DiagnosticsDebug.WriteLine(string.Format(System.Globalization.CultureInfo.InvariantCulture, "Slot: {0} Row: {1} ", row.Slot, row.Index));
                }
                else
                {
                    DataGridRowGroupHeader groupHeader = element as DataGridRowGroupHeader;
