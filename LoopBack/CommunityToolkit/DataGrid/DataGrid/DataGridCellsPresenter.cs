...
namespace CommunityToolkit.WinUI.Controls.Primitives
{
    ...
    public sealed partial class DataGridCellsPresenter : Panel
    {
        ...
        internal void EnsureFillerVisibility()
        {
            ...
            if (OwningRow.FillerCell.Visibility != newVisibility)
            {
                OwningRow.FillerCell.Visibility = newVisibility;
                if (newVisibility == Visibility.Visible)
                {
                    OwningRow.FillerCell.Arrange(new Rect(_fillerLeftEdge, 0, fillerColumn.FillerWidth, ActualHeight));
                }
            }
        ...
        internal void Recycle()
        {
            ...
            {
                foreach (DataGridColumn column in OwningGrid.ColumnsInternal)
                {
                    OwningRow.Cells[column.Index].Recycle();
                }
            }
        }
        ...
        private bool ShouldDisplayCell(DataGridColumn column, double frozenLeftEdge, double scrollingLeftEdge)
        {
            ...
            DiagnosticsDebug.Assert(OwningGrid.HorizontalAdjustment >= 0, "Expected owning positive DataGrid.HorizontalAdjustment.");
            DiagnosticsDebug.Assert(OwningGrid.HorizontalAdjustment <= OwningGrid.HorizontalOffset, "Expected owning DataGrid.HorizontalAdjustment smaller than or equal to DataGrid.HorizontalOffset.");
