...
namespace CommunityToolkit.WinUI.Automation.Peers
{
    ...
    public partial class DataGridAutomationPeer :
        ...
        ITableProvider
    {
        private readonly Dictionary<object, DataGridGroupItemAutomationPeer> _groupItemPeers = [];
        private readonly Dictionary<object, DataGridItemAutomationPeer> _itemPeers = [];
            ...
            : base(owner)
        {
            if (HorizontallyScrollable)
            {
                ...
                _oldHorizontalScrollPercent = HorizontalScrollPercent;
                _oldHorizontalViewSize = HorizontalViewSize;
            }
            else
            {
            ...
            if (VerticallyScrollable)
            {
                ...
                _oldVerticalScrollPercent = VerticalScrollPercent;
                _oldVerticalViewSize = VerticalViewSize;
            }
            else
            {
        ...
        private bool HorizontallyScrollable => OwningDataGrid.HorizontalScrollBar != null && OwningDataGrid.HorizontalScrollBar.Maximum > 0;
        ...
        private double HorizontalScrollPercent
        {
            get
            {
                ...
                return (double)(OwningDataGrid.HorizontalScrollBar.Value * 100.0 / OwningDataGrid.HorizontalScrollBar.Maximum);
            }
        }
        ...
        private double HorizontalViewSize
        {
            get
            {
                if (!HorizontallyScrollable || DoubleUtil.IsZero(OwningDataGrid.HorizontalScrollBar.Maximum))
                {
                    return 100.0;
                }

                return (double)(OwningDataGrid.HorizontalScrollBar.ViewportSize * 100.0 /
                    (OwningDataGrid.HorizontalScrollBar.ViewportSize + OwningDataGrid.HorizontalScrollBar.Maximum));
            }
        }

        private DataGrid OwningDataGrid => Owner as DataGrid;

        private bool VerticallyScrollable => OwningDataGrid.VerticalScrollBar != null && OwningDataGrid.VerticalScrollBar.Maximum > 0;
        ...
        private double VerticalScrollPercent
        {
            get
            {
                ...
                return (double)(OwningDataGrid.VerticalScrollBar.Value * 100.0 / OwningDataGrid.VerticalScrollBar.Maximum);
            }
        }
        ...
        private double VerticalViewSize
        {
            get
            {
                if (!VerticallyScrollable || DoubleUtil.IsZero(OwningDataGrid.VerticalScrollBar.Maximum))
                {
                    return 100.0;
                }

                return (double)(OwningDataGrid.VerticalScrollBar.ViewportSize * 100.0 /
                    (OwningDataGrid.VerticalScrollBar.ViewportSize + OwningDataGrid.VerticalScrollBar.Maximum));
            }
        }
        ...
        protected override IList<AutomationPeer> GetChildrenCore()
        {
            ...
            {
                children.Remove(FromElement(OwningDataGrid.HorizontalScrollBar));
                children.Remove(FromElement(OwningDataGrid.VerticalScrollBar));
            }
        ...
        int IGridProvider.RowCount => OwningDataGrid.DataConnection.Count;

        IRawElementProviderSimple IGridProvider.GetItem(int row, int column)
        {
            if (OwningDataGrid != null &&
                OwningDataGrid.DataConnection != null &&
                row >= 0 && row < OwningDataGrid.SlotCount &&
                column >= 0 && column < OwningDataGrid.Columns.Count)
            {
                object item = null;
                if (!OwningDataGrid.IsSlotVisible(OwningDataGrid.SlotFromRowIndex(row)))
                {
                    item = OwningDataGrid.DataConnection.GetDataItem(row);
                }

                OwningDataGrid.ScrollIntoView(item, OwningDataGrid.Columns[column]);

                DataGridRow dgr = OwningDataGrid.DisplayData.GetDisplayedRow(row);
                if (OwningDataGrid.ColumnsInternal.RowGroupSpacerColumn.IsRepresented)
                {
                    column++;
                }

                DiagnosticsDebug.Assert(column >= 0, "Expected positive column value.");
                DiagnosticsDebug.Assert(column < OwningDataGrid.ColumnsItemsInternal.Count, "Expected smaller column value.");
        ...
        double IScrollProvider.HorizontalScrollPercent => HorizontalScrollPercent;

        double IScrollProvider.HorizontalViewSize => HorizontalViewSize;

        bool IScrollProvider.VerticallyScrollable => VerticallyScrollable;

        double IScrollProvider.VerticalScrollPercent => VerticalScrollPercent;

        double IScrollProvider.VerticalViewSize => VerticalViewSize;
        ...
        void IScrollProvider.Scroll(ScrollAmount horizontalAmount, ScrollAmount verticalAmount)
        {
            ...
            switch (horizontalAmount)
            {
                ...
                case ScrollAmount.SmallIncrement:
                    OwningDataGrid.ProcessHorizontalScroll(ScrollEventType.SmallIncrement);
                    break;
                case ScrollAmount.LargeIncrement:
                    OwningDataGrid.HorizontalScrollBar.Value += OwningDataGrid.HorizontalScrollBar.LargeChange;
                    OwningDataGrid.ProcessHorizontalScroll(ScrollEventType.LargeIncrement);
                    break;
                case ScrollAmount.SmallDecrement:
                    OwningDataGrid.ProcessHorizontalScroll(ScrollEventType.SmallDecrement);
                    break;
                case ScrollAmount.LargeDecrement:
                    OwningDataGrid.HorizontalScrollBar.Value -= OwningDataGrid.HorizontalScrollBar.LargeChange;
                    OwningDataGrid.ProcessHorizontalScroll(ScrollEventType.LargeDecrement);
                    break;
            ...
            switch (verticalAmount)
            {
                ...
                // ScrollBar.Value itself, so we don't need to do it here
                case ScrollAmount.SmallIncrement:
                    OwningDataGrid.ProcessVerticalScroll(ScrollEventType.SmallIncrement);
                    break;
                case ScrollAmount.LargeIncrement:
                    OwningDataGrid.VerticalScrollBar.Value += OwningDataGrid.VerticalScrollBar.LargeChange;
                    OwningDataGrid.ProcessVerticalScroll(ScrollEventType.LargeIncrement);
                    break;
                case ScrollAmount.SmallDecrement:
                    OwningDataGrid.ProcessVerticalScroll(ScrollEventType.SmallDecrement);
                    break;
                case ScrollAmount.LargeDecrement:
                    OwningDataGrid.VerticalScrollBar.Value -= OwningDataGrid.VerticalScrollBar.LargeChange;
                    OwningDataGrid.ProcessVerticalScroll(ScrollEventType.LargeDecrement);
                    break;
                case ScrollAmount.NoAmount:
                    break;
                default:
        ...
        void IScrollProvider.SetScrollPercent(double horizontalPercent, double verticalPercent)
        {
            ...
            {
                OwningDataGrid.HorizontalScrollBar.Value =
                    (double)(OwningDataGrid.HorizontalScrollBar.Maximum * (horizontalPercent / 100.0));
                OwningDataGrid.ProcessHorizontalScroll(ScrollEventType.ThumbPosition);
            }

            if (scrollVertically)
            {
                OwningDataGrid.VerticalScrollBar.Value =
                    (double)(OwningDataGrid.VerticalScrollBar.Maximum * (verticalPercent / 100.0));
                OwningDataGrid.ProcessVerticalScroll(ScrollEventType.ThumbPosition);
            }
        }
        ...
        IRawElementProviderSimple[] ISelectionProvider.GetSelection()
        {
                ...
                OwningDataGrid.SelectedItems != null)
            {
                ...
                foreach (object item in OwningDataGrid.SelectedItems)
                {
        ...
        {
            if (OwningDataGrid.AreColumnHeadersVisible)
            {
                List<IRawElementProviderSimple> providers = [];
                foreach (DataGridColumn column in OwningDataGrid.Columns)
                {
        ...
        {
            if (OwningDataGrid.AreRowHeadersVisible)
            {
                List<IRawElementProviderSimple> providers = [];
                foreach (DataGridRow row in OwningDataGrid.DisplayData.GetScrollingElements(true /*onlyRows*/).OfType<DataGridRow>())
                {
        ...
        {
            if (slot >= 0 && slot < OwningDataGrid.SlotCount &&
                column >= 0 && column < OwningDataGrid.ColumnsItemsInternal.Count &&
                OwningDataGrid.IsSlotVisible(slot))
            {
                DataGridRow row = OwningDataGrid.DisplayData.GetDisplayedElement(slot) as DataGridRow;
                if (row != null)
                {
                    DiagnosticsDebug.Assert(column >= 0, "Expected positive column value.");
                    DiagnosticsDebug.Assert(column < OwningDataGrid.ColumnsItemsInternal.Count, "Expected smaller column value.");
        ...
        internal List<AutomationPeer> GetChildPeers()
        {
            ...
            {
                foreach (object group in OwningDataGrid.DataConnection.CollectionView.CollectionGroups /*Groups*/)
                {
        ...
        internal DataGridGroupItemAutomationPeer GetOrCreateGroupItemPeer(object group)
        {
            ...
            if (group != null)
            {
                if (_groupItemPeers.TryGetValue(group, out DataGridGroupItemAutomationPeer value))
                {
                    peer = value;
                }
                else
                {
                    peer = new DataGridGroupItemAutomationPeer(group as ICollectionViewGroup, OwningDataGrid);
        ...
        internal DataGridItemAutomationPeer GetOrCreateItemPeer(object item)
        {
            ...
            if (item != null)
            {
                if (_itemPeers.TryGetValue(item, out DataGridItemAutomationPeer value))
                {
                    peer = value;
                }
                else
                {
                    peer = new DataGridItemAutomationPeer(item, OwningDataGrid);
        ...
        {
            Dictionary<object, DataGridGroupItemAutomationPeer> oldChildren = new(_groupItemPeers);
            _groupItemPeers.Clear();

            if (OwningDataGrid.DataConnection.CollectionView != null &&
#if FEATURE_ICOLLECTIONVIEW_GROUP
                this.OwningDataGrid.DataConnection.CollectionView.CanGroup &&
#endif
                OwningDataGrid.DataConnection.CollectionView.CollectionGroups != null &&
                OwningDataGrid.DataConnection.CollectionView.CollectionGroups.Count > 0)
            {
                List<object> groups = [.. OwningDataGrid.DataConnection.CollectionView.CollectionGroups];
                while (groups.Count > 0)
                {
                    ...
                    if (cvGroup != null)
                    {
                        ...
                        {
                            peer = oldChildren[cvGroup];
                        }
                        else
                        {
                            peer = new DataGridGroupItemAutomationPeer(cvGroup, OwningDataGrid);
                        }
        ...
        {
            Dictionary<object, DataGridItemAutomationPeer> oldChildren = new(_itemPeers);
            _itemPeers.Clear();

            if (OwningDataGrid.ItemsSource != null)
            {
                foreach (object item in OwningDataGrid.ItemsSource)
                {
                    if (item != null)
                    {
                        DataGridItemAutomationPeer peer;
                        if (oldChildren.TryGetValue(item, out DataGridItemAutomationPeer value))
                        {
                            peer = value;
                        }
                        else
        ...
        internal void RaiseAutomationFocusChangedEvent(int slot, int column)
        {
            if (slot >= 0 && slot < OwningDataGrid.SlotCount &&
                ...
                OwningDataGrid.IsSlotVisible(slot))
            {
                if (OwningDataGrid.RowGroupHeadersTable.Contains(slot))
                {
                    DataGridRowGroupHeader header = OwningDataGrid.DisplayData.GetDisplayedElement(slot) as DataGridRowGroupHeader;
                    if (header != null)
                    {
        ...
        internal void RaiseAutomationSelectionEvents(SelectionChangedEventArgs e)
        {
            ...
            if (ListenerExists(AutomationEvents.SelectionItemPatternOnElementSelected) &&
                OwningDataGrid.SelectedItems.Count == 1)
            {
                if (OwningDataGrid.SelectedItem != null && _itemPeers.TryGetValue(OwningDataGrid.SelectedItem, out DataGridItemAutomationPeer peer))
                {
#if DEBUG_AUTOMATION
        ...
        internal void UpdateRowGroupHeaderPeerEventsSource(DataGridRowGroupHeader header)
        {
            ...
            DataGridRowGroupHeaderAutomationPeer peer = FromElement(header) as DataGridRowGroupHeaderAutomationPeer;
        ...
        internal void UpdateRowPeerEventsSource(DataGridRow row)
        {
            ...
            if (peer != null && row.DataContext != null && _itemPeers.TryGetValue(row.DataContext, out DataGridItemAutomationPeer value))
            {
                peer.EventsSource = value;
            }
        }
    }
}